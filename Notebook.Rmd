...
Our primary objective of this analysis is to identify the main factors and reasons why students are less likely to pursue higher education indicated by "will_go_to_college" being 'False'. By leveraging the provided dataset with attributes such as (school type, school accreditation, gender, interest in college, and residence, we aim to discover the most influential variables and their relationships with the decision not to attend college.
...
-   The class label: The class label in our project is the attribute "will_go_to_college". This attribute is binary, meaning it can take on two values: True for yes or False for no. The value of this attribute will be the target variable we are trying to predict during our project.
...
+-----------------------+------------------------------------------------------------+-------------+-------------------------------------+
| gender                | The student's gender                                       | Nominal     | Male / Female                       |
+-----------------------+------------------------------------------------------------+-------------+-------------------------------------+
...
Here is a sample of 6 rows from our dataset.
...
```{r}
ggplot(Dataset, aes(x = type_school, fill = will_go_to_college)) +
  geom_bar(position = "dodge") +
  labs(title = "Student's Type of School vs College Attendance",
       x = "Type of School",
       y = "Count") +
  scale_fill_manual(values = c("True" = "antiquewhite2", "False" = "antiquewhite2")) +
  theme_minimal()
...
This information tells us that a higher proportion of students from academic high schools are going to college compared to those from vocational schools which suggests that the type of school attended.

Graph 4:

```{r}
ggplot(Dataset, aes(x = average_grades)) +
  geom_histogram(binwidth = 5, fill = "antiquewhite3", color = "antiquewhite4") +
...
This histogram shows us that the majority of the students in the dataset are performing well since it seems like the grades span between 75 and 98. This analysis will help us determine whether the academic performance level of students is a contributing factor to their college attendance or not.
...
Additionally, we can utilize the house area attribute to gain a deeper understanding of the socioeconomic status of students' families, where students with houses significantly larger than the mean might indicate a higher socioeconomic status, while those with houses considerably smaller than the mean might reflect a comparatively lower socioeconomic status. Based on the shown output, the house areas range from [20.00-120.00 ㎡]. The median house area is 75.50 ㎡ indicating that families with house areas around this value likely have moderate socioeconomic status with houses that are neither very small nor very large.
...
After conducting data analysis and identifying outliers, our inspection reveals that the detected outliers represent inherent variation within the population. Regarding Parent_age, outliers are observed for values below 44 and above 65. However, it should be noted that the minimum value is 40 and the maximum is 65, which falls within the normal range of outliers. For parent_salary, we found two outliers: one below 1,326,250 ind ≈ 85 USD and another above 9,416,250 ind ≈ 606 USD. The minimum and maximum values were determined to be 1,000,000 ind ≈ 64 USD and 10,000,000 ind ≈ 644 USD, respectively. In the case of grades, twelve outliers were identified, ranging from below 76 to above 97. Nevertheless, since the data falls within the acceptable range of 0 to 100, these outliers should be retained as they are still considered normal and within the usual grade range. Finally, for house_area, we found eleven outliers below 34.4m and above 115m, with the minimum being 20m and the maximum being 120m. However, these values are still considered typical for the population.
...
After conducting a thorough analysis of the correlation, we determined that the correlation between the variables is significantly low. As a result, we reject the null hypothesis, indicating that the attributes are independent of each other.

## Discretization

```{r}
Dataset$parent_age<-cut(Dataset$parent_age, seq(40,65,by=10),right = FALSE)
```

We transformed the parent_age attribute into intervals by dividing the values to fall on one of two possible interval labels with equal width which is(40,50],(50,60] by discretization the values will be simpler to classify or perform other methods that can help us later in our model.

## Encoding to be corrected
...
Dataset$interest[Dataset$interest=="Very Interested"]<-2
Dataset$interest[Dataset$interest=="Less Interested"]<-1
...
print(DatasetWithoutNormalization)
Dataset$parent_salary<-normalize(DatasetWithoutNormalization$parent_salary)
print(Dataset)
```

We also normalized the attribute parent_salary and house_area to take values between 0 and 1. Which will also improve the reliability of our data analysis.
