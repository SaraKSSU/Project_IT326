...
Here is a sample of 6 row from our dataset.
...
ggplot(Dataset, aes(x = type_school, fill = will_go_to_college)) +
  ...
  labs(title = "Student's Type of School vs College Attendance",
       x = "Type of School",
       y = "Count") +
  scale_fill_manual(values = c("True" = "antiquewhite3", "False" = "antiquewhite2")) +
  theme_minimal()
...
After conducting data analysis and identifying outliers, our inspection reveals that the detected outliers represent inherent variation within the population. Regarding Parent_age, outliers are observed for values below 44 and above 65. However, it should be noted that the minimum value is 40 and the maximum is 65, which fall within the normal range of outliers. For parent_salary, we found two outliers: one below 1,326,250 ind ≈ 85 USD and another above 9,416,250 ind ≈ 606 USD. The minimum and maximum values were determined to be 1,000,000 ind ≈ 64 USD and 10,000,000 ind ≈ 644 USD, respectively. In the case of grades, twelve outliers were identified, ranging from below 76 to above 97. Nevertheless, since the data falls within the acceptable range of 0 to 100, these outliers should be retained as they are still considered normal and within the usual grade range. Finally, for house_area, we found eleven outliers below 34.4m and above 115m, with the minimum being 20m and the maximum being 120m. However, these values are still considered typical for the population. \## Correlatiin analysis Chi square test for nominal attribute:
...
All the attributes has X-square greater than the p-valuse, therfore we will reject the null hypothesis except for the residence that has chi-square smaller than the p-value.
...
```{r}
X <- Dataset[, c("gender","type_school","school_accreditation","interest","residence","average_grades","parent_age","parent_salary","parent_was_in_college","house_area")]
...
To determine the relative importance of columns with respect to the class label, we employed a code that ranks the columns. The ranking is based on their significance in predicting the class label, with the most important columns listed first and the least important columns listed last. This approach allows us to prioritize the columns that have a stronger influence on the class label and aids us in focusing our analysis on the most relevant features. \## Normalization
...
Dataset$parent_age=cut(Dataset$parent_age,breaks = breaks)
